...
func (_q *BlobQuery) QueryVersions() *ObjectVersionQuery {
...
func (_q *BlobQuery) WithVersions(opts ...func(*ObjectVersionQuery)) *BlobQuery {
	query := (&ObjectVersionClient{config: _q.config}).Query()
...
func (_q *BlobQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Blob, error) {
	...
	if query := _q.withVersions; query != nil {
		if err := _q.loadVersions(ctx, query, nodes,
			func(n *Blob) { n.Edges.Versions = []*ObjectVersion{} },
			func(n *Blob, e *ObjectVersion) { n.Edges.Versions = append(n.Edges.Versions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *BlobQuery) loadVersions(ctx context.Context, query *ObjectVersionQuery, nodes []*Blob, init func(*Blob), assign func(*Blob, *ObjectVersion)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(objectversion.FieldBlobID)
	}
	query.Where(predicate.ObjectVersion(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(blob.VersionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "blob_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
